...
var mainSector = traveltime.NewSector(570, -3094, "Main")
...
func TestDistance(t *testing.T) {
	for key, data := range distanceData {
		testSector := traveltime.NewSector(data.x, data.y, "")
		expected := data.fn()
...
func TestSecondRounding(t *testing.T) {
	for key, data := range roundingData {
		result := traveltime.RoundToSecond(data.value)
...
func TestTravelTime(t *testing.T) {
	for key, data := range travelData {
		destination := traveltime.NewSector(data.x, data.y, "Test")
